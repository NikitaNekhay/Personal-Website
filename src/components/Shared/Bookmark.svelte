...
<script lang="ts">
	/**
	 * Reusable "bookmark" menu — a playful tab that slides in from the right edge,
	 * sways "in the wind", and on click morphs into a panel of pills laid out in a
	 * single row. Behaviour (auto-reveal cycle, hover-peek, corner detection) is
	 * section-agnostic; callers just pass the list of items.
	 *
	 * Originally the home-page collection filter — now shared so any section (e.g.
	 * /works) can drop in its own row of choices.
	 */
	import { onMount } from 'svelte';

	interface Props {
		items: BookmarkItem[];
		/** Accessible label for the collapsed tab button. */
		ariaLabel?: string;
		/** Text that peeks out on hover; defaults to the first item's label. */
		peekLabel?: string;
		/** Collapse the panel shortly after a selection (default true). */
	...
	const peek = $derived(peekLabel ?? items[0]?.label ?? '');
	...
	function selectItem(item: BookmarkItem) {
		item.onSelect();
...
<div class="bookmark-wrap" class:is-in={tabVisible || menuOpen}>
	...
	>
		{#if menuOpen}
			<nav class="bookmark-nav" aria-label={ariaLabel}>
				{#each items as item}
					<button
						type="button"
						class="bookmark-pill"
		...
		{:else}
			...
			<!-- При наведении закладка чуть подрастает и из-за края выглядывает
			     первый пункт — намёк на спрятанное меню. -->
			<span class="peek" aria-hidden="true">{peek}</span>
		{/if}
	</div>
</div>
...
<style>
	...
	/* Выглядывающий первый пункт — виден только при наведении. */
	.peek {
		position: absolute;
		left: 1.35rem;
		top: 50%;
	...
	/* Наведение на свёрнутую закладку: чуть подрастает, грип прячется,
	   проявляется краешек первого пункта. */
